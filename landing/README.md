...
- 공개 다운로드: **v0.1.4**. x64·Arm64 통합 설치 파일과 개별 설치 파일을 제공합니다.
- 최신 기능: 구간 편집·이름 변경·재생 속도·품질 프리셋을 공개 릴리스 기능으로 안내합니다.
...
`vite.config.mjs`가 같은 정보로 HTML 메타데이터를 생성합니다. `index.html`을 별도로 고칠 필요는 없습니다. 새 버전을 공개할 때는 최신 기능 영역·FAQ와 루트 README도 검토합니다. 파일 크기는 실제 릴리스 자산의 바이트 수를 십진 MB로 반올림합니다.
