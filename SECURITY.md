...
Release `v0.1.1` is not Authenticode-signed. Windows SmartScreen may therefore show an unrecognized-app warning even when the downloaded checksum matches the published `SHA256SUMS.txt`. A trusted code-signing certificate remains a release-hardening priority.
