...
코덱별 입력·출력 계수를 기록해 영상은 출력되지만 오디오 변환기가 약 5초를 버퍼링하는 문제를 추가로 확인했다. 실시간 캡처에서는 불필요한 변환을 제거하고 트랙의 실제 샘플레이트·채널 수를 사용한다.

CI는 기본 48 kHz 스테레오와 `--mono-audio`의 44.1 kHz 모노를 각각 검사한다. 실제 인코더 설정이 입력과 일치하고 첫 오디오 입력이 1초 미만인지도 확인해 수초 단위 변환 버퍼가 다시 유입되지 않도록 한다.
